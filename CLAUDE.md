...
- **Six tools**, not one per resource: `netbox_global_search`, `netbox_discover`, `netbox_describe`, `netbox_read`, `netbox_write`, `netbox_invoke`. Object types are derived at runtime from the instance's own OpenAPI schema; `netbox_invoke` exposes only schema-confirmed, closed IPAM prefix availability/allocation and DCIM cable trace/path actions.
