 ...
 *
 * Six tools in four layers, replacing one tool per operation:
 *
 ...
 *      netbox_invoke         closed semantic IPAM prefix actions
 *
 ...
 * tool in a surface of six. netbox_invoke is deliberately one mixed-action
 * tool; its per-operation classification is returned as structured metadata.
 *
...
export function registerLayeredTools(
  ...
  registerInvoke(server, schema, api, errorSanitizer);
}
