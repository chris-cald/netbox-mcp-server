...
describe.skipIf(process.env.NETBOX_E2E !== "1")(
  "patched NetBox 4.6.7 available IPs fixture",
  () => {
    it("validates the nullable-role upstream patch, preserves native reads, and allocates one persisted IP without retrying", async () => {
      ...
      await runner.withNetBoxFixture(async ({ url, token, prefix, logPath }) => {
        ...
        const schema = (await schemaResponse.json()) as {
          components?: { schemas?: { IPAddress?: { properties?: { role?: unknown } } } };
        };
